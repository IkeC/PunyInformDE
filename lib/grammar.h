...
#Endif;
	...
	print " / Inform v";
	inversion;
	print " PunyInformDE v", PUNYINFORM_MAJOR_VERSION, (char) '.', PUNYINFORM_MINOR_VERSION;
...
! String appended after room contents list (e.g. " here.^"). Overridable.
#IfNDef LISTOBJS_ROOM_SUFFIX; Constant LISTOBJS_ROOM_SUFFIX = " here.^"; #EndIf;
...
[ _ListObjsInOnMsg p_parent;
	...
	print (the) p_parent, " you can see ";
	if(also_flag) print "also ";
];
#EndIf;
...
#Endif;
	...
	while(_ceil ~= player or 0) {
		...
		if(parent(_ceil) == 0) {
			...
			_you_can_see_2 = LISTOBJS_ROOM_SUFFIX;
		} else {
